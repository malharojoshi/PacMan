...
function draw() {
  ...
  if (gameState === "start") {
    ...
    for (var q = 0; q < 26; q++) {
      if (ghost1 && ghost1.collide(cardboards[q])) {
        direction = Math.round(random(1, 4));
        randomDirection(ghost1, direction);
      }
      if (ghost2 && ghost2.collide(cardboards[q])) {
    ...
    if (ghost1 && coin1 && ghost1.isTouching(coin1)) {
      coin1.remove();
      coin1 = null;
    }
    if (ghost1 && coin2 && ghost1.isTouching(coin2)) {
      coin2.remove();
      coin2 = null;
    }
    if (ghost1 && coin3 && ghost1.isTouching(coin3)) {
      coin3.remove();
      coin3 = null;
    }
    if (ghost2 && coin1 && ghost2.isTouching(coin1)) {
      coin1.remove();
      coin1 = null;
    }
    if (ghost2 && coin2 && ghost2.isTouching(coin2)) {
      coin2.remove();
      coin2 = null;
    }
    if (ghost2 && coin3 && ghost2.isTouching(coin3)) {
      coin3.remove();
      coin3 = null;
    }
    ...
    if (ghost1 && pacMan.isTouching(ghost1)) {
      ghost1.remove();
      ghost1 = null;
    }
    if (ghost2 && pacMan.isTouching(ghost2)) {
      ghost2.remove();
      ghost2 = null;
    }
    if (!ghost1 && !ghost2) {
  ...
  if (pacMan && gameState === "endLoss") {
    console.log(gameState);
    text("You Lost", 190, 150);

    pacMan.velocityX = 0;
    pacMan.velocityY = 0;
  }
  if (!ghost1 && !ghost2 && pacMan && gameState === "endWon") {
    console.log(gameState);
    text("You Won", 190, 150);
    ghost1.velocityX = 0;
    ghost1.velocityY = 0;
    ghost2.velocityX = 0;
    ghost2.velocityY = 0;
